...
contract("SupplyChain", function (accounts) {
  ...
  describe("Variables", () => {
    ...
    describe("enum State", () => {
      ...
      it("should define `Shipped`", () => {
        assert(enumState.hasOwnProperty("Shipped"), "The enum does not have a `Shipped` value");
      });

      it("should define `Received`", () => {
        assert(enumState.hasOwnProperty("Received"), "The enum does not have a `Received` value");
      });
    });
    ...
    describe("Item struct", () => {
      ...
      before(() => {
        ...
        assert(subjectStruct !== null, "The contract should define an `Item Struct`");
      });

      it("should have a `name`", () => {
        assert(isDefined(subjectStruct)("name"), "Struct Item should have a `name` member");
        assert(isType(subjectStruct)("name")("string"), "`name` should be of type `string`");
      });

      it("should have a `sku`", () => {
        assert(isDefined(subjectStruct)("sku"), "Struct Item should have a `sku` member");
        assert(isType(subjectStruct)("sku")("uint"), "`sku` should be of type `uint`");
      });

      it("should have a `price`", () => {
        assert(isDefined(subjectStruct)("price"), "Struct Item should have a `price` member");
        assert(isType(subjectStruct)("price")("uint"), "`price` should be of type `uint`");
      });

      it("should have a `state`", () => {
        assert(isDefined(subjectStruct)("state"), "Struct Item should have a `state` member");
        assert(isType(subjectStruct)("state")("State"), "`state` should be of type `State`");
      });

      it("should have a `seller`", () => {
        assert(isDefined(subjectStruct)("seller"), "Struct Item should have a `seller` member");
        assert(isType(subjectStruct)("seller")("address"), "`seller` should be of type `address`");
        assert(isPayable(subjectStruct)("seller"), "`seller` should be payable");
      });

      it("should have a `buyer`", () => {
        assert(isDefined(subjectStruct)("buyer"), "Struct Item should have a `buyer` member");
        assert(isType(subjectStruct)("buyer")("address"), "`buyer` should be of type `address`");
  ...
  describe("Use cases", () => {
    it("should add an item with the provided name and price", async () => {
      ...
      assert.equal(result[3].toString(10), SupplyChain.State.ForSale, 'the state of the item should be "For Sale"');
    ...
    it("should allow someone to purchase an item and update state accordingly", async () => {
      ...
      assert.equal(result[3].toString(10), SupplyChain.State.Sold, 'the state of the item should be "Sold"');
    ...
    it("should allow the seller to mark the item as shipped", async () => {
      ...
      assert.equal(result[3].toString(10), SupplyChain.State.Shipped, 'the state of the item should be "Shipped"');
    });
    ...
    it("should allow the buyer to mark the item as received", async () => {
      ...
      assert.equal(result[3].toString(10), SupplyChain.State.Received, 'the state of the item should be "Received"');
    });
